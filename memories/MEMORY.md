Name: Mojtaba. Associate degree in Emergency Medical Services. Learning German A2 (AnkiDroid). Planning migration for Pflege Ausbildung. Casual/friendly. Telegram. Espresso enthusiast (Western 8860). Shares tokens (warn don't block).
§
Backup: cron ID 55f3a242af3b (12h), script ~/.hermes/scripts/hermes_backup.sh -> GitHub mojtabaalimohamadi4122/hermesbackup. Script uses cp fallback (rsync missing).
§
...
§
Language tutoring prefs: Use gTTS (not edge TTS - too robotic for German). Format: word -> phonetic (Persian script) -> meaning -> grammar table -> examples -> audio -> AnkiDroid tip. User studies A2 German for nursing migration (Ausbildung Pflegefachmann).
§
